...
ICON_GENERATION_TARGET="${1:-all}"

if [[ "$ICON_GENERATION_TARGET" != "all" && "$ICON_GENERATION_TARGET" != "legacy" && "$ICON_GENERATION_TARGET" != "native" ]]; then
  echo "Icon generation target must be all, legacy, or native." >&2
  exit 1
fi

if [[ "$ICON_GENERATION_TARGET" != "native" ]]; then
...
if [[ "$ICON_GENERATION_TARGET" != "legacy" ]] && ! xcrun --find actool >/dev/null 2>&1; then
  echo "Xcode's actool is required to generate the macOS app icon asset catalog." >&2
  exit 1
fi

if [[ "$ICON_GENERATION_TARGET" == "all" ]]; then
...
if [[ "$ICON_GENERATION_TARGET" != "legacy" ]]; then
